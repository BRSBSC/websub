...
在 Side Panel 中一键总结当前页面正文，支持 OpenAI 兼容 API、Kimi 免 Key 与 Qwen 国际版免 Key 三种模式。
...
- 三提供商并存
  ...
  - `Qwen 国际版（免 Key）`（通过 Qwen 网页登录态 token）
...
1. 打开扩展 Side Panel，进入“设置”
2. 提供商选择 `Qwen 国际版（免 Key）`
3. 点击“连接 Qwen”
4. 在打开的 `https://chat.qwen.ai/` 页面完成登录
5. 回到扩展，保存设置后即可在“总结”页直接使用
...
- 登录连接：需先点击“连接 Qwen”并在 `https://chat.qwen.ai/` 完成登录
- 本地持久化：token 保存在本地 `chrome.storage.local`
- 失效处理：登录态失效时会清理 token 并提示  
  `登录状态失效，请重新连接 Qwen`
- 首版能力：仅文本总结（不含文件上传/解析路径）
- 不会自动回退到 OpenAI：需要用户主动重连 Qwen

## 历史记录兼容

- 新记录会保存 `provider` 字段（`openai`、`kimi_web` 或 `qwen_web`）
...
- Qwen 国际版模式依赖 Qwen 网页接口与网页登录态实现
- 若 Qwen 官方网页接口或字段变化（如 `localStorage.token`、接口路径），功能可能失效
